...
/*struct GaussOutput { //Used online resources to see how best to get these terms from the Gauss function to the Get_position_vectors function
    double D0;
...
void IOD::Find_State(int measurement_set){
...
std::vector<double> IOD::Gauss(const IOD_Measurements& Measurement1, const IOD_Measurements& Measurement2, const IOD_Measurements& Measurement3, double mu, GaussOutput& output) {
    ...
    std::vector<double> r2= root_finder(a, b, c);
    output.r2  = r2;
    output.A = A;
    output.B = B;
    output.tau1 = tau1;
...
/*
const std::vector<double>& getPositionVectors() {
    return r2_vec;
}
