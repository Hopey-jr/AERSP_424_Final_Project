...
void Create_Transfers(IOD& state1, Arrival_Orbit& arrival_orbit, Logger *logger){
    ...
    logger->log(FUNCTION_START,"The positive and negative manifolds will begin to be found");
    ...
    logger->log(FUNCTION_START,"The initial conditions for the positive and negative manifolds will begin to be found");
    ...
    logger->log(FUNCTION_END,"The initial conditions for the positive and negative manifolds have been to be found");
        ...
        double poincare_section_x = -mu;
            ...
            for (int i = 0; i < num_threads; ++i) {
                ...
                threads.push_back(std::thread(process_negative_manifold, start_index, end_index, std::ref(ICs_Stable_Negative),
                                              mu, poincare_section_x, std::ref(stable_negative_state)));
            }
            ...
            for (int i = 0; i < num_threads; ++i) {
                ...
                threads.push_back(std::thread(process_positive_manifold, start_index, end_index, std::ref(ICs_Stable_Positive),
                                              mu, poincare_section_x, std::ref(stable_positive_state)));
            }
            // Join all threads
            for (auto& t : threads) {
                t.join();
    ...
    logger->log(FUNCTION_END,"The positive and negative manifolds have been to be found");
    ...
    */
    ...
    for( int i = 0; i < stable_positive_state.size(); ++i){
    ...
    for( int i = 0; i < stable_negative_state.size(); ++i){
    ...
    if( y_pos_min > y_neg_min){
        logger->log(POS_VS_NEG,"The negative manifold will be used for the transfer");

        for(int number_of_stable_manifolds = 0; number_of_stable_manifolds < stable_negative_state.size(); number_of_stable_manifolds++){
                ...
                T2.push_back(hour*stable_negative_state[number_of_stable_manifolds][6]);

            
        }
        
    }else{
        logger->log(POS_VS_NEG,"The positive manifold will be used for the transfer");
        for(int number_of_stable_manifolds = 0; number_of_stable_manifolds < stable_positive_state.size(); number_of_stable_manifolds++){
                ...
                T2.push_back(hour*stable_positive_state[number_of_stable_manifolds][6]);
                
            
        }
    }
    std::ostringstream message_Poincare_hit;
    message_Poincare_hit << "x= "<< poincare_section_x << ": "<< stable_manifold.size() << " out of " << number_of_manifolds << " branches" << '\n';
    logger->log(HIT_POINCARE,message_Poincare_hit.str());
     ...
     */
    logger->log(FUNCTION_START,"The check to determine if any branches of the manifold hit the Moon will begin");
    {
        int number_of_hits = 0;
        std::vector<std::thread> threads_moon;
        int num_threads_moon = std::thread::hardware_concurrency(); // Use available hardware threads
        
        int chunk_size_moon = stable_manifold.size() / num_threads_moon;
        
        // Launch threads
        for (int i = 0; i < num_threads_moon; ++i) {
            int start_moon = i * chunk_size_moon;
            int end_moon = (i + 1) * chunk_size_moon;
            // int end = (t == num_threads - 1) = stable_manifold.size(); // : start + chunk_size;
            if (i == num_threads_moon - 1) end_moon = stable_manifold.size(); // Handle remainder
            threads_moon.push_back(std::thread(Check_If_Hits_Moon, std::ref(stable_manifold), std::ref(hits_moon), start_moon, end_moon, abs_err, rel_err, LU, mu));
         

        }
        
        // Join threads
        for (auto& thread : threads_moon) {
            thread.join();
        }
    }
    logger->log(FUNCTION_END,"The check to determine if any branches of the manifold hit the Moon has finished");
    std::ostringstream message_Moon_hit;
    message_Moon_hit << hits_moon.size() << " out of " << stable_manifold.size() << " branches of the stable manifold" << '\n';
    logger->log(HIT_MOON,message_Moon_hit.str());
...
while(good_first_guess == 1){
    ...
    std::vector<std::vector<double>> x_vectors_all;
    std::vector<std::vector<double>> y_vectors_all;
    std::vector<std::vector<double>> z_vectors_all;
        ...
        while (j < max_iterations && TOF > 0 && err < 10){
            ...
            if (std::abs(err) < tol) {
                ...
                if(hits_earth_moon == 0 && TOF > 0 ){
                    ...
                    T2_elapsed.push_back(T2[i]+TOF*hour);
                    
                    
                    std::ostringstream message_Delta_V;
                    double total_delta_v  = std::sqrt(pow(delta_vx.back(),2)+pow(delta_vy.back(),2)+pow(delta_vz.back(),2));
                    message_Delta_V << "The total delta_v needed for the transfer is: " << total_delta_v << "LU/TU. The total time is takes to transfer from the current position of the spacecraft to the arrival orbit is: " << T2_elapsed.back() << '\n';
                    logger->log(MAX_DELTA_V,message_Delta_V.str());
                    ...
                    if(std::sqrt(pow(delta_vx.back(),2)+pow(delta_vy.back(),2)+pow(delta_vz.back(),2)) < 10 ){
                        ...
                        std::vector<double> x_transfer;
                        std::vector<double> y_transfer;
                        std::vector<double> z_transfer;

                        for(int i = 0; i < transfer_state.size(); i++){
                            x_transfer.push_back(transfer_state[i][0]);
                            y_transfer.push_back(transfer_state[i][1]);
                            z_transfer.push_back(transfer_state[i][2]);
                        }
                        
                        x_vectors_all.push_back(x_transfer);
                        y_vectors_all.push_back(y_transfer);
                        z_vectors_all.push_back(z_transfer);
            ...
            TOF = TOF + 0.95*DF(3,0);
            //]
            std::ostringstream message_iteration;
            message_iteration <<  i << ". Iteration number: " << j << " with error: " << err << '\n';
            logger->log(BRANCH_COUNT,message_iteration.str());
...
//        cout << "Manifold Branch Number: " << i << endl;
    ...
    //[ Plotting the periodic orbit
    std::vector< state_type > Orbit_State =  Plot_Periodic_Orbit(arrival_orbit);
    std::vector<double> x_PO;
    std::vector<double> y_PO;
    std::vector<double> z_PO;

    for(int i = 0; i < Orbit_State.size(); i++){
        x_PO.push_back(Orbit_State[i][0]);
        y_PO.push_back(Orbit_State[i][1]);
        z_PO.push_back(Orbit_State[i][2]);
    }
    x_vectors_all.push_back(x_PO);
    y_vectors_all.push_back(y_PO);
    z_vectors_all.push_back(z_PO);
